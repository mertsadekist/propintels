...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    interface ConfRow { bucket: string; cnt: bigint; }
    const confBuckets = await prisma.$queryRaw<ConfRow[]>`
      SELECT CASE
    ...
    return NextResponse.json({
      data: {
        ...
        scatter:       scatter.map((r) => ({
          ...
          marketPsf:    r.transactionMedianPsf != null ? Number(r.transactionMedianPsf) : null,
          verdict:      r.verdict,
          ratio:        r.ratioToMarket != null ? Number(r.ratioToMarket) : null,
        })),
        summary: {
          totalLeads,
          totalResults,
    ...
    console.error("[valuations] error:", e);
    return NextResponse.json(
