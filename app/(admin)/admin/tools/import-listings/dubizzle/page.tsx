...
export default function DubizzleImportPage() {
  ...
  async function handleFetch() {
    ...
    setBlocked(false);
    try {
      const res = await fetch("/api/tools/dubizzle-scrape", {
        method: "POST",
      ...
      if (data.blocked) {
        ...
        setFetchMessage(
          data.message ??
            "Request blocked by Dubizzle (Incapsula). Please try again later or enter data manually."
        );
        return;
      }
      ...
      if (!data.listings?.length) {
        setFetchMessage(
          data.message ?? "No listings found on this page. Make sure the URL is a Dubizzle search results page."
        );
        return;
      }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Import Listings from Dubizzle</h1>
        <p className="text-sm text-gray-500 mt-1">
          Extract property listings from a Dubizzle search URL and add them as entries to a selected
          project.
        </p>
      </div>
      ...
      {step === 1 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Step 1 — Enter the Dubizzle URL and target project</CardTitle>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="flex gap-2 bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
              ...
              <span>
                Scraping data from Dubizzle may violate their Terms of Service. This tool is for
                internal use only and is used at your own risk.
              </span>
            </div>

            {/* URL */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700">Dubizzle Search URL</label>
              <Input
                placeholder="https://dubai.dubizzle.com/en/property-for-sale/residential/?building=10708"
                value={url}
                onChange={(e) => setUrl(e.target.value)}
                className="font-mono text-xs"
              />
              <p className="text-xs text-gray-400">
                Copy the URL from Dubizzle after searching for the project or location you want to
                import. Both for-sale and for-rent pages are supported.
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-white border border-gray-200 rounded-lg p-4">
            <div>
              <p className="font-semibold text-gray-900">
                {listings.length} listings extracted from Dubizzle
              </p>
            ...
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Portal: Dubizzle — review them in the project&apos;s Entries page.
                </p>
              </div>
